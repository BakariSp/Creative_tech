	...
	void main() {
			...
			for (int i = 0; i < ${MAX_ATTRACTOR_COUNT}; i++) {
				if (i < attractorCount) {
					...
					float dis = distance(st, attractorPos.xy);
					value = attractorMass[i]  * 0.0002 * float(i) / distance(st, attractorPos.xy);	
					// Multiplier may need to be adjusted if max attractor count is tweaked.
